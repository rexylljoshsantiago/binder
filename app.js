        ...
        if (view.lessonId) {
          ...
          if (lesson) {
            ...
            if (view.screen === "lesson") {
              actions = `<button class="icon-btn" title="Export lesson as PDF" onclick="exportLessonPDF()">⇣</button>
                         <button class="icon-btn" title="Edit lesson" onclick="Editor.openLessonEditor('${subj.id}','${lesson.id}')">✎</button>`;
            } else {
              actions = "";
            }
          }
...
async function exportLessonPDF() {
  ...
  if (lesson && lesson.title) {
    document.title = lesson.title.replace(/[\\/:*?"<>|]/g, "").trim() || "Lesson";
  }
